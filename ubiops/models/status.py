...
class Status(object):
    ...
    openapi_types = {"status": "str", "reason": "str"}
    ...
    def reason(self, reason):
        ...
        if self.client_side_validation and (reason is not None and not isinstance(reason, str)):
            raise ValueError("Parameter `reason` must be a string")

        if self.client_side_validation and (reason is not None and len(reason) < 1):
